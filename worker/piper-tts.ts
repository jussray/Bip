...
function getPiperVoice(characterId: PiperCharacterId, env: PiperTtsEnv): string {
  const configured = characterId === 'raylene'
    ? env.PIPER_RAYLENE_VOICE
    : characterId === 'rylane'
      ? env.PIPER_RYLANE_VOICE
      : characterId === 'cloud'
        ? env.PIPER_CLOUD_VOICE
        : characterId === 'night'
          ? env.PIPER_NIGHT_VOICE
          : characterId === 'parentCoach'
            ? env.PIPER_PARENT_COACH_VOICE
...
export async function synthesizeWithPiper(input: {
  ...
  env: PiperTtsEnv;
}): Promise<{ bytes: Uint8Array; contentType: string; voice: string } | null> {
