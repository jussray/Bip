...
import { generateInviteCode } from '@/utils/parentLink';
...
export default function ParentLinkVerifyScreen() {
  ...
  async function createCode() {
    setLoading(true);
    setError('');
    const nextCode = await generateInviteCode();
    await refreshVerification();
    setLoading(false);
    if (!nextCode) {
      setError('Could not create an invite code. Try again.');
      return;
    }
    setCode(nextCode);
  }
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={styles.content}>
        ...
        <TouchableOpacity style={styles.primary} onPress={createCode} disabled={loading}>
          {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.primaryText}>{code ? 'Create a new code' : 'Create invite code'}</Text>}
        </TouchableOpacity>

        <TouchableOpacity style={styles.secondary} onPress={checkStatus} disabled={loading}>
