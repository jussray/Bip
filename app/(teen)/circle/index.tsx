...
export default function TeenCircleRoute() {
  const { theme, mood, selectedSekret, crewMembers, setCrewMembers, crewCheckIns, setCrewCheckIns } = useAppContext();
  const t = THEME_PACKS[theme] ?? THEME_PACKS.neon;
  const [tab, setTab] = useState<Tab>('circle');
  const [circleName, setCircleName] = useState('anonymous bip');

  useEffect(() => {
    loadTeenCircleIdentity().then(value => setCircleName(value.circleName));
  }, []);
  ...
  return (
    <View style={s.root}>
      <View style={s.tabs}>
        <TouchableOpacity style={[s.tab, tab === 'circle' && s.tabActive]} onPress={() => setTab('circle')}>
          <Text style={[s.tabText, tab === 'circle' && s.tabTextActive]}>🌐 Circle</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[s.tab, tab === 'crew' && s.tabActive]} onPress={() => setTab('crew')}>
          <Text style={[s.tabText, tab === 'crew' && s.tabTextActive]}>🤝 Crew</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[s.tab, tab === 'messages' && s.tabActive]} onPress={() => setTab('messages')}>
          <Text style={[s.tabText, tab === 'messages' && s.tabTextActive]}>💜 Messages</Text>
        </TouchableOpacity>
        <TouchableOpacity style={s.weatherBtn} onPress={() => router.push('/(teen)/circle/weather' as any)}>
          <Text style={s.weatherBtnText}>🌧️</Text>
        </TouchableOpacity>
        <TouchableOpacity style={s.profileBtn} onPress={() => router.push('/(teen)/profile' as any)}>
          <Text style={s.profileBtnText}>🪪 {circleName}</Text>
        </TouchableOpacity>
      </View>
...
const s = StyleSheet.create({
  ...
  profileBtn: { maxWidth: 110, paddingHorizontal: 10, paddingVertical: 12, alignItems: 'center', justifyContent: 'center' },
