...
// ---------------------------------------------------------------------------
// Pattern library
// Ordered by specificity. Patterns are intentionally broad to catch metaphor
// and indirect expression. Context is everything — err on the side of support.
// ---------------------------------------------------------------------------
...
const TRIGGER_PATTERNS: TriggerPattern[] = [
  // High — direct self-harm or suicide ideation
  {
  ...
  // High — explicit abuse/assault disclosure
  {
  ...
  // Medium — hopelessness, giving up, indirect ideation
  {
...
// ---------------------------------------------------------------------------
// Response messages — warm, non-clinical, non-alarming
// ---------------------------------------------------------------------------
...
export const safetyTrigger = {
  /**
   * Check user text for safety signals.
   * Safe to call on every user input — designed to be fast and non-blocking.
   */
  check(text: string): SafetyTriggerResult {
    ...
    for (const trigger of TRIGGER_PATTERNS) {
      if (trigger.pattern.test(normalised)) {
    ...
    // Highest level wins
    const topLevel = _highestLevel(matched.map((m) => m.level));
    ...
    return {
      ...
      _matchedPatterns: matched.map((m) => m.label),
    };
  },

  /**
   * Check asynchronously — use for server-side augmentation hooks.
   * Currently wraps the sync check; extend to call a backend classifier here.
   */
