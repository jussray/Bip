...
### 10.3 Found — column mismatch fixed in code; secret mismatch still needs an operator
...
1. **Shared-secret mismatch — still open, needs an operator.** The Edge Function returned `401 unauthorized` on the real request. The Vault secret `safety_scan_secret` *is* configured (created 2026-06-23), so either the Edge Function's `SAFETY_SCAN_SECRET` env var was never set, or the two values don't match. This can't be fixed from code — it requires someone with Supabase CLI/dashboard access to rotate both sides to the same value:
   ```
   supabase secrets set SAFETY_SCAN_SECRET=<new-random-32+-char-value>
   -- then in the SQL editor:
   select vault.update_secret((select id from vault.secrets where name = 'safety_scan_secret'), '<same-new-value>');
   ```
2. **`safety_alerts`'s real live columns don't match what the Edge Function inserted — fixed.** Live columns: `id, teen_user_id, parent_user_id, source_mood_id, source_post_id, alert_type, severity, title, summary, is_read, created_at, updated_at, scan_metadata`. The function used to insert `user_id, alert_type, source_table, source_id, severity, scan_metadata` — `user_id` doesn't exist (it's `teen_user_id`), `source_table` doesn't exist at all, `source_id` doesn't exist (there's `source_post_id`/`source_mood_id` instead, a typed-per-source design rather than a generic table+id pair). Rewritten in `supabase/functions/safety-scan/index.ts` to: write `teen_user_id`; resolve `parent_user_id` via an active `parent_links` lookup before insert (the parent-read RLS policy checks the stored column, not a join); populate `source_post_id` only for the `posts` table (the only source with a typed FK slot today — `journal_entries`/`circle_posts`/`public_circle_posts` have none yet, so their origin stays in `scan_metadata` instead); and generate parent-safe `title`/`summary` copy (severity/category only, never source content). The client-side counterpart of the same bug — `src/features/safety/safetyCoordinator.ts`'s `checkForFlaggedItems()`, which queried `safety_alerts` with the same stale `user_id`/`parent_notified_at` columns and would silently return `null` (no `safety_flagged` experience ever shown to the teen) — was fixed the same way.

The column-mismatch fix has not been exercised against the live database in this session (no live DB access here) — recommend a real end-to-end test (as done in this section) once the secret is rotated.
