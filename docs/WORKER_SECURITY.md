...
This client value is extractable from the app bundle. It is an interim speed bump, not user identity. Supabase JWT verification is the planned replacement.

`BIP_CLIENT_TOKEN` and `PIPER_API_TOKEN` are different values for different boundaries.
...
`OPTIONS` requests are not authenticated or rate limited.

## Rate limits

- replies: 30 per minute
- voice synthesis: 12 per minute
- transcription: 8 per minute

Bindings are configured in `wrangler.toml`. Current keys combine route and Cloudflare client IP. Replace the IP portion with a verified Supabase user ID after JWT auth is added.
