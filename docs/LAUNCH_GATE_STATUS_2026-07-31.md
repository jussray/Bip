...
| Cloudflare dashboard | Current dashboard state must be re-checked when release/deployment evidence is material; prior unauthenticated Wrangler observations are not current provider truth | re-check when needed |
