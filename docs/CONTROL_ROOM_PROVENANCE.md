...
- Canonical JSON rejects unsupported, undefined, or non-finite values.
...
The repository's existing `npm test` recursively discovers every `test/**/*.test.mjs` file, so the new provenance tests automatically enter the unit-test gate without creating a parallel test runner.

## Verification status

Implementation status is `integrated` on the review branch.

Do not mark this capability `verified` or `released` until exact-head CI succeeds for the branch and the reviewed content is merged to `main`.
