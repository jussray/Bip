...
def _find_metric(rows: list[list[Any]], aliases: set[str]) -> int | None:
...
def parse_workbook(path: Path) -> dict[str, Any]:
    ...
    followers = _find_metric(sheets["FOLLOWERS"], {"total followers", "followers"})
...
def main() -> int:
    ...
    try:
        ...
        return 0
    except Exception as error:  # fail closed with a single actionable message
