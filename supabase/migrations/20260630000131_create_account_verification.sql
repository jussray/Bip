...
drop trigger if exists account_verification_set_updated_at
on public.account_verification;

create trigger account_verification_set_updated_at
before update on public.account_verification
for each row
execute function public.set_account_verification_updated_at();
...
drop trigger if exists initialize_account_verification_on_signup
on auth.users;

create trigger initialize_account_verification_on_signup
after insert on auth.users
for each row
execute function public.initialize_account_verification();
