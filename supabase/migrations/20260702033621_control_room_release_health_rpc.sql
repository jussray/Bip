...
declare
  ...
  v_summary jsonb;
begin
  select * into v_release from public.control_room_releases where release_key = p_release_key;
  if v_release.id is null then raise exception 'Unknown release key: %', p_release_key; end if;
  select * into v_previous from public.control_room_releases where deployed_at < v_release.deployed_at order by deployed_at desc limit 1;
  update public.control_room_issues set linked_release = v_release.release_key, updated_at = now()
  ...
  select count(*),
         ...
         count(*) filter (where occurrence_count > 1 or severity in ('error','critical'))
  into v_issue_count, v_error_count, v_warning_count, v_regression_count
  from public.control_room_issues
  where linked_release = v_release.release_key and status not in ('resolved','ignored');
  v_summary := jsonb_build_object(
    ...
    'health', case when v_error_count > 0 then 'degraded' when v_warning_count > 0 then 'watch' else 'healthy' end
  );
  ...
  set issue_count = v_issue_count,
      ...
      warning_count = v_warning_count,
      regression_count = v_regression_count,
      status = case when v_error_count > 0 then 'degraded' when v_warning_count > 0 then 'watch' else 'healthy' end,
      summary = v_summary,
      updated_at = now()
