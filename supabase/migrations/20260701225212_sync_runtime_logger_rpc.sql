...
begin
  ...
  v_metadata := coalesce(p_metadata, '{}'::jsonb)
    - 'journalText' - 'journal_text' - 'rawAudio' - 'raw_audio'
    - 'audioBlob' - 'audio_blob' - 'token' - 'access_token'
    - 'refresh_token' - 'apiKey' - 'api_key' - 'authorization'
    - 'conversation' - 'conversationText' - 'conversation_text'
    - 'transcript' - 'fullTranscript' - 'full_transcript'
    - 'messageText' - 'message_text' - 'content' - 'payload';
  insert into public.audit_events (user_id,event_type,screen,severity,message,metadata)
  values (v_effective_user,btrim(p_event_type),nullif(btrim(p_screen),''),p_severity,left(p_message,500),v_metadata)
