...
create policy bridge_share_requests_teen_select on public.bridge_share_requests for select to authenticated using (teen_user_id = auth.uid());
drop policy if exists bridge_share_requests_teen_insert on public.bridge_share_requests;
create policy bridge_share_requests_teen_insert on public.bridge_share_requests for insert to authenticated with check (
  teen_user_id = auth.uid() and exists (
...
create policy bridge_share_requests_teen_update on public.bridge_share_requests for update to authenticated using (teen_user_id = auth.uid()) with check (teen_user_id = auth.uid());
drop policy if exists bridge_share_requests_parent_select on public.bridge_share_requests;
create policy bridge_share_requests_parent_select on public.bridge_share_requests for select to authenticated using (
...
create policy bridge_share_sources_teen_select on public.bridge_share_sources for select to authenticated using (
  exists (
    select 1 from public.bridge_share_requests r
    where r.id = bridge_share_sources.request_id and r.teen_user_id = auth.uid()
  )
);
drop policy if exists bridge_share_sources_teen_insert on public.bridge_share_sources;
create policy bridge_share_sources_teen_insert on public.bridge_share_sources for insert to authenticated with check (
  exists (
    select 1 from public.bridge_share_requests r
    where r.id = bridge_share_sources.request_id and r.teen_user_id = auth.uid() and r.status = 'pending' and r.revoked_at is null
  )
);
drop policy if exists bridge_summaries_teen_select on public.bridge_summaries;
create policy bridge_summaries_teen_select on public.bridge_summaries for select to authenticated using (
  exists (
    select 1 from public.bridge_share_requests r
    where r.id = bridge_summaries.request_id and r.teen_user_id = auth.uid()
  )
);
drop policy if exists bridge_summaries_parent_select on public.bridge_summaries;
create policy bridge_summaries_parent_select on public.bridge_summaries for select to authenticated using (
  exists (
    select 1 from public.bridge_share_requests r
    join public.parent_links pl on pl.teen_user_id = r.teen_user_id and pl.parent_user_id = r.parent_user_id
...
create policy bridge_summary_views_parent_select on public.bridge_summary_views for select to authenticated using (parent_user_id = auth.uid());
drop policy if exists bridge_summary_views_parent_insert on public.bridge_summary_views;
create policy bridge_summary_views_parent_insert on public.bridge_summary_views for insert to authenticated with check (
  parent_user_id = auth.uid()
  and exists (
    select 1 from public.bridge_summaries s
...
create policy bridge_delivery_preferences_owner_all on public.bridge_delivery_preferences for all to authenticated using (teen_user_id = auth.uid()) with check (teen_user_id = auth.uid());
...
declare
  ...
  v_source jsonb;
begin
  if v_teen_user_id is null then raise exception 'unauthorized'; end if;
  if p_parent_user_id is null or p_parent_user_id = v_teen_user_id then raise exception 'invalid_parent'; end if;
  if p_idempotency_key is null or length(trim(p_idempotency_key)) < 8 then raise exception 'invalid_idempotency_key'; end if;
  if jsonb_typeof(p_sources) <> 'array' or jsonb_array_length(p_sources) = 0 then raise exception 'sources_required'; end if;
  if jsonb_array_length(p_sources) > 20 then raise exception 'too_many_sources'; end if;
  if not exists (
    select 1 from public.parent_links pl
    where pl.teen_user_id = v_teen_user_id
      ...
      and pl.status = 'active'
      and pl.is_active = true
  ) then raise exception 'active_parent_link_required'; end if;

  insert into public.bridge_share_requests (teen_user_id, parent_user_id, status, idempotency_key, consented_at, expires_at)
  values (v_teen_user_id, p_parent_user_id, 'pending', trim(p_idempotency_key), now(), p_expires_at)
  on conflict (teen_user_id, idempotency_key) do update set updated_at = now()
  returning id into v_request_id;

  for v_source in select value from jsonb_array_elements(p_sources)
  loop
    if coalesce(v_source->>'kind','') not in ('journal','mood','goal','scrapbook') or nullif(trim(v_source->>'sourceId'),'') is null then raise exception 'invalid_source'; end if;
...
begin
  ...
  where id = p_request_id and teen_user_id = v_teen_user_id and status not in ('revoked','deleted');
  return found;
end;
$$;
...
comment on table public.bridge_share_requests is 'Teen-created consent record for a generated parent-safe Bridge summary. Does not contain raw source content.';
comment on table public.bridge_share_sources is 'References exact teen-selected sources. Parent policies intentionally expose none of these rows.';
comment on table public.bridge_summaries is 'Generated summary content only; readable by the teen and the currently active linked parent while unrevoked.';
