...
function fingerprint(item) {
  return `github_actions:${item.repository}:pr-${item.pr_number}:${item.workflow_id}:${item.head_sha}`;
}
...
async function collectFailures(repo) {
  ...
  failures.sort((a, b) => String(b.updated_at).localeCompare(String(a.updated_at)));
  return failures;
}
...
async function ingestFailure(item) {
  ...
  const inserted = await supabaseRequest('/rest/v1/audit_events', {
    ...
    body: JSON.stringify({
      ...
      screen: `github-pr-${item.pr_number}`,
      severity: item.severity,
      message: `${item.workflow_name} failed for PR #${item.pr_number} at ${item.head_sha.slice(0, 12)}.`,
      metadata,
      resolved: false,
    }),
  });
  ...
  await supabaseRequest('/rest/v1/rpc/upsert_control_room_issue', {
    ...
    body: JSON.stringify({
      ...
      p_status: 'open',
      p_title: `${item.workflow_name} failed on PR #${item.pr_number}`,
