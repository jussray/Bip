...
before relying on them. If the repository has changed, update this skill in the same PR or open
...
1. Open `context/` and confirm the current context names and ownership boundaries.
...
- Supabase Realtime may carry live membership/post updates; verify current subscriptions and RLS.
...
- Identity-bearing fields must not enter client state for audiences that are not authorized to receive them.
- Parent Bridge data must remain consent-based and summary-only at the response boundary.

## Deploy Pipeline
The intended deployment paths are:
- Mobile: Expo EAS Build / Update according to the target channel and native-change requirements.
...
- Instead, document the available local/preview/test path, run all applicable automated checks, and require explicit production-deploy approval.
