...
export function isApprovedGuardianLink(
  link: Pick<ParentTeenLink, 'status'> | null | undefined,
): boolean {
...
export function canGuardianAccessSharedContent(
  ...
  return (
    ...
    link.permissions.includes(shareKind)
  );
}
...
export async function createTeenGuardianInvite(): Promise<string | null> {
  ...
  const { error } = await sb.from(TABLES.parentTeenInvites).upsert(
    { teen_id: teenId, invite_code: inviteCode, status: 'pending' },
...
export async function updateGuardianLinkStatus(
  ...
  const { error } = await sb
    ...
    .update({ status, permissions: status === 'approved' ? permissions : [], updated_at: new Date().toISOString() })
    .eq('id', linkId);
  if (error) throw error;
}
