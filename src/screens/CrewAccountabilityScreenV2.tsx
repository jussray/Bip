...
type CrewProfileRow = {
  ...
  display_name: string;
  avatar_emoji: string;
  identity_visibility: 'accepted_crew';
...
async function loadAcceptedCrew(): Promise<AcceptedCrewMember[]> {
  ...
  const profileMap = new Map<string, CrewProfileRow>(
    ((profiles ?? []) as CrewProfileRow[]).map(profile => [profile.user_id, profile]),
  );
...
export function CrewAccountabilityScreen() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const hasLiveData = accepted.length > 0
        || (mine.ok && mine.value.length > 0)
        || (shared.ok && shared.value.length > 0);
  ...
  function toggleAll() {
    setSelectedIds(current => current.size === members.length
      ? new Set()
  ...
  async function encourage(item: CrewFeedItem, presetKey: string) {
    if (previewSample) {
      setFeed(current => current.map(entry => entry.shareId === item.shareId
        ? {
            ...entry,
            encouragementCount: entry.myEncouragementKey
              ? entry.encouragementCount
              : entry.encouragementCount + 1,
    ...
    <ScrollView style={styles.root} contentContainerStyle={styles.content}>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionLabel}>Share with accepted Crew · {members.length}</Text>
          {members.length > 0 ? (
            <TouchableOpacity onPress={toggleAll}>
              <Text style={styles.selectAll}>{selectedIds.size === members.length ? 'clear all' : 'select all'}</Text>
            </TouchableOpacity>
          ) : null}
        </View>
      ...
      <View style={styles.card}>
        ...
        {feed.length === 0 ? <Text style={styles.empty}>Nothing shared with you yet.</Text> : feed.map(item => {
          const sender = identityMap.get(item.ownerUserId);
          return (
