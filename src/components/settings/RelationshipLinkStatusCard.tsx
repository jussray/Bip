...
interface RelationshipLinkStatusCardProps {
  accountSide: ParentLinkAccountSide;
  refreshKey?: number;
  onRevoked?: () => void;
}

function relationshipCopy(link: ParentLinkStatusSnapshot): string {
  ...
  if (link.status === 'revoked') return 'This relationship has been unlinked.';
  if (link.status === 'expired') return 'This invitation expired before it was accepted.';
  return 'Relationship status is unavailable.';
}
...
export function RelationshipLinkStatusCard({
  accountSide,
  refreshKey = 0,
  onRevoked,
}: RelationshipLinkStatusCardProps) {
  ...
  const confirmUnlink = useCallback((linkId?: string) => {
    const exactRelationship = Boolean(linkId);
    Alert.alert(
      `Unlink ${counterparty}?`,
      exactRelationship
        ? `Linked access will be removed immediately. A new invite is required to reconnect.`
        : `Relationship status is unavailable. This will ask the secure server to remove your current parent link without guessing from cached UI state.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            setUnlinkingId(linkId ?? 'current-teen-link');
            ...
            if (!result.value) {
              Alert.alert('Already unlinked', 'No active or pending relationship was found. Refresh status to confirm.');
              await loadStatus();
              return;
            }
  ...
  return (
    <View style={styles.panel}>
      <Text style={styles.heading}>{counterparty === 'parent' ? 'Parent' : 'Teen'} relationship status</Text>

      {loadState.kind === 'loading' && (
        <View style={styles.loadingRow}>
          <ActivityIndicator color="#a78bfa" />
          <Text style={styles.body}>Loading secure relationship status…</Text>
        </View>
      )}
      ...
      {loadState.kind === 'error' && (
        <View>
          ...
          {accountSide === 'teen' && (
            <>
              <Text style={styles.warning}>A teen has one canonical parent-link row, so you can still request server-side unlinking while status is unavailable.</Text>
              <TouchableOpacity
          ...
          {accountSide === 'parent' && (
            <Text style={styles.warning}>For safety, retry before unlinking so the app does not guess which teen relationship you meant.</Text>
          )}
        </View>
      )}

      {loadState.kind === 'ready' && loadState.links.length === 0 && (
        <Text style={styles.body}>No {counterparty} relationship is recorded for this account.</Text>
      )}

      {loadState.kind === 'ready' && loadState.links.map((link, index) => {
        const updated = updatedCopy(link.updatedAt);
        return (
          <View key={link.linkId} style={styles.relationshipRow}>
            <Text style={styles.statusLabel}>
              {loadState.links.length > 1 ? `Relationship ${index + 1}: ` : ''}{link.status}
            </Text>
            <Text style={styles.body}>{relationshipCopy(link)}</Text>
...
const styles = StyleSheet.create({
  ...
  relationshipRow: { borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.10)', paddingTop: 10, marginTop: 8 },
