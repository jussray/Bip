            ...
            return (
              <View key={mem.id} style={styles.memberCard}>
                <View style={styles.memberHeader}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={styles.memberName}>{mem.connectionStatus === 'accepted' ? mem.name : (mem.bipId || 'pending Bip ID')}</Text>
                    <Text style={styles.memberCommit}>{mem.commitment}</Text>
                    <Text style={styles.memberMeta}>
                      {mem.connectionStatus === 'accepted' ? 'trusted crew' : mem.connectionStatus === 'blocked' ? 'blocked' : 'pending invite'} · Bip ID: {mem.bipId || mem.inviteCode}
                    </Text>
                  </View>
              ...
              {crewCheckIns.slice(0, 8).map(c => {
                ...
                return (
                  <View key={c.id} style={styles.activityRow}>
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={styles.activityName}>{mem?.connectionStatus === 'accepted' ? mem.name : (mem?.bipId || 'crew member')}</Text>
