...
// Se'kret Bip — Bippin2 (Growth Space)
//
// Fixes applied (2026-06-03):
//   A1 — growthPath/setGrowthPath moved to internal state; art prop removed
//         (index.tsx does NOT pass these — component was crashing on mount)
//   A2 — onMilestone?: () => void added to interface (index.tsx passes this)
//   B1 — streakDays?: number prop accepted (replaces hardcoded '7 days'/'9 days')
//   B2 — stub feature items now call onMilestone?.() so growth is tracked
//   C1 — roomWrap View gets pointerEvents="box-none" so hotspots work on Android
//   D1 — dead `art` prop removed from interface
...
// ── Props ─────────────────────────────────────────────────────────────────
// Fix A1: growthPath/setGrowthPath are now INTERNAL state — index.tsx does
//         not pass them. art removed (was never used).
// Fix A2: onMilestone added — index.tsx passes () => trackActivity('growth')
// Fix B1: streakDays optional — falls back to 0 if not passed
...
export function Bippin2Screen({
  ...
  // Fix A1: internal growth path state
  const [growthPath, setGrowthPath] = useState<'girl' | 'boy' | 'preferNotToSay'>('preferNotToSay');
    ...
    <View style={[styles.root, { backgroundColor: t.background }]}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <TouchableOpacity
          ...
          onPress={() => setGrowthPath('girl')}
        >
          <Text style={styles.btnText}>🌙 Womanhood</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.btn, { backgroundColor: t.accent }]}
          onPress={() => setGrowthPath('boy')}
        >
  ...
  // Fix B2: stub items call onMilestone?.() so growth activity is tracked
  const featureItems = isGirl ? [
    { e: '🩸', l: 'first period support', fn: () => { onMilestone?.(); } },
    { e: '🌙', l: 'cycle wellness',        fn: () => setScreen('periodCalendar') },
    { e: '💗', l: 'mood + body check-in',  fn: () => { onMilestone?.(); } },
    { e: '🪷', l: 'comfort mode',           fn: () => setScreen('comfort') },
    { e: '☁️', l: "ask Se'kret",            fn: () => setScreen('sekret') },
    { e: '🔒', l: 'private journal',        fn: () => setScreen('pages') },
  ] : [
    { e: '🧍🏾', l: 'puberty guide',       fn: () => { onMilestone?.(); } },
    { e: '💪🏾', l: 'body changes',        fn: () => { onMilestone?.(); } },
    { e: '⭐',   l: 'confidence boost',    fn: () => { onMilestone?.(); } },
    { e: '🧴',   l: 'hygiene + self-care', fn: () => { onMilestone?.(); } },
    { e: '🧠',   l: 'mind check-in',       fn: () => { onMilestone?.(); } },
    { e: '🔒',   l: 'private journal',     fn: () => setScreen('pages') },
  ];

  // Fix B1: real streak count from prop, labeled correctly
  const streakLabel = streakDays === 1
    ? '1 day'
    : streakDays > 0
    ? `${streakDays} days`
    : 'just started';
  ...
  return (
    <View style={[styles.root, { backgroundColor: '#0d0914' }]}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {/* ── Interactive Room ── */}
        {/* Fix C1: pointerEvents="box-none" so touches pass through to hotspots on Android */}
        <View style={styles.roomWrap} pointerEvents="box-none">
          ...
          <TouchableOpacity
            ...
            style={[
              styles.hotspot,
              { top: HOTSPOTS.calendar.top, right: HOTSPOTS.calendar.right, width: HOTSPOTS.calendar.width, height: HOTSPOTS.calendar.height },
          ...
          <TouchableOpacity
            ...
            style={[
              styles.hotspot,
              { top: HOTSPOTS.stickyNotes.top, left: HOTSPOTS.stickyNotes.left, width: HOTSPOTS.stickyNotes.width, height: HOTSPOTS.stickyNotes.height },
              DEBUG_HOTSPOTS && styles.hotspotDebug,
            ]}
          ...
          <TouchableOpacity
            ...
            style={[
              styles.hotspot,
              { bottom: HOTSPOTS.teddyBear.bottom, right: HOTSPOTS.teddyBear.right, width: HOTSPOTS.teddyBear.width, height: HOTSPOTS.teddyBear.height },
              DEBUG_HOTSPOTS && styles.hotspotDebug,
            ]}
          ...
          <TouchableOpacity
            ...
            style={[
              styles.hotspot,
              { top: HOTSPOTS.cloud.top, left: HOTSPOTS.cloud.left, width: HOTSPOTS.cloud.width, height: HOTSPOTS.cloud.height },
              DEBUG_HOTSPOTS && styles.hotspotDebug,
            ]}
          ...
          {!DEBUG_HOTSPOTS && (
            <>
              ...
              <View style={[styles.hint, { bottom: '26%', left: '28%' }]} pointerEvents="none">
        ...
        <View style={styles.duoRow}>
          <View style={[styles.polaroid, { borderColor: t.accent, backgroundColor: 'rgba(13,9,20,0.85)' }]}>
            ...
            {/* Fix B1: real streak from prop */}
            <Text style={[styles.polaroidBig, { color: t.accent }]}>{streakLabel}</Text>
          ...
          <View style={[styles.polaroid, { borderColor: t.accent, backgroundColor: 'rgba(13,9,20,0.85)' }]}>
            ...
            <Text style={[styles.polaroidBody, { color: '#E2E8F0' }]}>
              {isGirl
                ? "Your body isn't something to hate. It's becoming YOU."
                : "Confidence isn't loud. It's built quietly every day."}
            </Text>
            <TouchableOpacity
        ...
        <View style={styles.overlayWrap}>
          ...
          <View style={[styles.overlayCard, { backgroundColor: 'rgba(13,9,20,0.96)', borderColor: t.accent }]}>
            ...
            <View style={styles.overlayRow}>
              <TouchableOpacity
                style={[styles.overlayBtn, { borderColor: t.accent }]}
        ...
        <View style={styles.overlayWrap}>
          ...
          <View style={[styles.overlayCard, { backgroundColor: 'rgba(13,9,20,0.96)', borderColor: t.accent }]}>
            ...
            <View style={styles.overlayRow}>
              <TouchableOpacity
                style={[styles.overlayBtn, { borderColor: t.accent }]}
                onPress={() => setComfortIdx(i => (i + 1) % COMFORT.length)}
              >
                <Text style={[styles.overlayBtnText, { color: t.soft }]}>another 💜</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => setOverlay(null)}>
