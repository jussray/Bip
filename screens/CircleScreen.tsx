...
function PublicFeed() {
  const [posts, setPosts] = useState<PublicCirclePost[]>(MOCK_PUBLIC);
...
function FriendsFeed({ myUserId }: { myUserId: string }) {
  const [posts, setPosts] = useState<FriendsCirclePost[]>(MOCK_FRIENDS);
...
function CrewFeed({ myUserId }: { myUserId: string }) {
  const [posts, setPosts] = useState<CrewCirclePost[]>(MOCK_CREW);
  const [commentTarget, setCommentTarget] = useState<number | null>(null);
  const [commentText, setCommentText] = useState('');
  const [comments, setComments] = useState<Record<number, CircleComment[]>>({});
...
function ParentFeed({ myUserId }: { myUserId: string }) {
  const [posts, setPosts] = useState<ParentCirclePost[]>(MOCK_PARENT);
  const [commentTarget, setCommentTarget] = useState<number | null>(null);
  const [commentText, setCommentText] = useState('');
  const [comments, setComments] = useState<Record<number, CircleComment[]>>({});
...
export default function CircleScreen() {
  // In production, resolve myUserId and myNickname from Supabase auth + circle_profiles.
  ...
  const handlePost = useCallback((tab: CircleTab, text: string) => {
    // TODO: insert into the appropriate Supabase table based on tab.
    // public → public_circle_posts (strip identity before insert, never include nickname)
    // friends → friends_circle_posts
    // crew → crew_circle_posts
    // parent → parent_circle_posts
    console.log('Post to', tab, ':', text);
  }, []);
