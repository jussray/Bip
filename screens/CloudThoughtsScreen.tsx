...
async function fetchSekretReply(text: string, context = 'cloud', mood?: string): Promise<string> {
  try {
...
export function CloudThoughtsScreen({ t, mood, setScreen, BottomNav }: CloudThoughtsScreenProps) {
  ...
  const sendThought = async () => {
    ...
    setSent(true);
    setIsThinking(true);
    const r = await fetchSekretReply(text, 'cloud', mood);
    setReply(r);
    setIsThinking(false);
  };
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {isThinking && (
          <View style={[styles.replyCard, { borderColor: 'rgba(168,85,247,0.3)', backgroundColor: 'rgba(13,9,20,0.9)' }]}>
            ...
            <Text style={[styles.thinkingText, { color: t.soft }]}>Raylene is sitting with that... ☁️</Text>
          </View>
        )}
        ...
        {reply && !isThinking && (
          <View style={[styles.replyCard, { borderColor: 'rgba(168,85,247,0.3)', backgroundColor: 'rgba(13,9,20,0.92)' }]}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={[styles.replyLabel, { color: '#a855f7' }]}>Raylene says 💜</Text>
        ...
        <View style={[styles.modeRow]}>
          {[
            { emoji: '🧠', label: 'brain dump',    sub: 'just let it all out' },
            { emoji: '🌙', label: 'night thoughts', sub: 'for when it gets loud' },
            { emoji: '💭', label: 'reflection',     sub: 'look back softly' },
            { emoji: '🫶', label: 'check-in',       sub: 'how are you really' },
          ].map(mode => (
            <TouchableOpacity
              key={mode.label}
              style={[styles.modeBtn, { borderColor: t.accent, backgroundColor: 'rgba(13,9,20,0.82)' }]}
              onPress={() => {
                setInput('');
                setReply('');
                setSent(false);
...
const styles = StyleSheet.create({
  ...
  modeRow:       { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginHorizontal: 16, marginBottom: 14 },
  modeBtn:       { width: '47%', borderRadius: 16, borderWidth: 1, padding: 14, alignItems: 'center' },
