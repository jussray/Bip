...
test('Circle and Bridge preserve separate product boundaries', () => {
  const policy = read('src/features/circle/audiencePolicy.ts');
  const purposes = read('src/constants/screenPurpose.ts');

  assert.equal(policy.includes('Bridge remains a'), true);
  assert.equal(policy.includes('separate private teen-to-parent product boundary'), true);
...
test('identity exposure requires active trust for Friends, Groups, and Crew', () => {
  const policy = read('src/features/circle/audiencePolicy.ts');
